
# 🚀 Hello, I'm Founder of RecursiveZero

Building scalable, production-ready web applications and exploring the frontiers of Computer Vision and Image Processing. We focus on clean architecture, minimal technical debt, and high-performance deployments.

-----

### 🛠️ Core Tech Stack

| **Area** | **Technologies** |
| :--- | :--- |
| **Frontend** | React.js, Astro, TypeScript, Tailwind CSS |
| **Backend** | Hono.js, Node.js, Python (Poetry), Express |
| **Cloud & DevOps** | AWS (IAM, CloudFront, WAF), GitHub Actions, Caddy, Ubuntu 24.04 |
| **AI/ML** | Computer Vision, Image Processing, OpenCV |

-----

### 🔭 Current Projects

  * **[ABCD KBD](https://abcdkbd.com):** An interactive learning platform for toddlers, rooted in the Indian cultural system and designed for early childhood development.
  * **[Tech Taboo](https://recursivezero.github.io/taboo/):** An interactive meetup game built with Alpine.js and Astro.
  * **[Tiny URL](https://rzro.link):** A high-performance URL shortener using Python and Fast API.
  * **[Threadzip](https://lab.threadzip.com):** An AI Chatbot for fabric and raw material industry.
  * **[Action-Club](https://github.com/recursivezero/action-club):** A collection of reusable GitHub Actions for rapid project scaffolding.

-----

### 🎓 Internship Opportunities

We are currently looking for **Front-End** and **Back-End/AI** interns to work on real-world, high-impact projects.

  * **Days:** Tue – Sat | **Sync:** 9 PM (Teams)
  * 👉 **[View Open Positions & Apply Here](https://github.com/recursivezero/recursivezero/discussions/25)**

-----

### 📫 Connect with Me

  * **Email:** [recursivezero@outlook.com](mailto:recursivezero@outlook.com)
  * **Environment:** Primary Dev OS: **Ubuntu 24.04** | IDE: **VS Code**



